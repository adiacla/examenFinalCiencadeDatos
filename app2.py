...
def evaluar_taller_redes_neuronales(contenido_estudiante: str):
    ...
    prompt = f"""
    Eres un profesor experto en Visión por Computador y ML.  
    Objetivo: evaluar un script .py que implementa un "piano virtual" (MediaPipe + pygame).
    La maxima nota es:1.5 puntos.  
    ...
    "detalle": [
        ...
        {{"criterio":"deteccion_landmarks","score":0.00,"comentario":""}},
        {{"criterio":"audio_y_recursos","score":0.00,"comentario":""}}
    ],
    "resumen": "Texto corto con evidencias y fallos detectados"
    }}

    Instrucciones de evaluación (breve):
    - Asigna score 0.00–1.50 (dos decimales) por criterio y escribe en "comentario" evidencias concretas (líneas de código, nombres de variables/funciones, mensajes de consola) que justifiquen la puntuación.
    - implementacion: archivo ejecutable, carga samples desde `sonidos/` (.mp3 preferido, .wav fallback), mapping de 10 notas (graves a la izquierda, agudos a la derecha), no reproducir sonidos al iniciar, presence de requirements.txt.
    - ejecucion: abre cámara (cv2.VideoCapture), muestra RAW (sin flip antes de process), reproduce sonido sin bloquear UI (p.ej. pygame.mixer.Sound.play), tiene cooldown por dedo, maneja errores de cámara/audio.
    - deteccion_landmarks: usa mp.solutions.hands, usa SOLO tips [4,8,12,16,20], usa coordenada y normalizada o umbral relativo, aplica suavizado (EMA u otro), umbral relativo (~0.03), produce lista de 10 entradas y maneja ausencia de mano (None).
    - audio_y_recursos: inicializa pygame.mixer con manejo de excepciones, carga mp3/wav desde `sonidos/`, si convierte usa pydub+verifica ffmpeg; muestra mensajes claros si faltan archivos o ffmpeg.
    - Penaliza: reproducción hardcodeada al inicio, uso de landmarks distintos a los tips para detectar pulsación, flip antes de procesar que confunda handedness sin compensación, dependencias faltantes sin instrucción.

    Salida: SOLO el JSON solicitado. A continuación el código del estudiante:
...
def evaluar_respuestas_abiertas(respuestas_estudiante):
    ...
    respuestas_esperadas = {
        1: "Devuelve el estado oculto final y permite encadenar decoders.",
        2: "Convierte índices enteros en vectores densos (embeddings).",
        3: "Porque son vectores muy grandes y es ineficiente.",
        4: "Una entrada secuencial y una salida única (ej: predicción de serie temporal).",
        5: "Una entrada secuencial y una salida secuencial (ej: traducción).",
        6: "Puede recordar información relevante por muchos pasos temporales.",
        7: "Porque depende del paso anterior, no se puede paralelizar por tiempo.",
        8: "Una red que reduce y luego reconstruye datos.",
        9: "El encoder comprime; el decoder reconstruye.",
        10: "Self-attention permite que cada token evalúe la relevancia de todos los demás tokens, capturando dependencias globales y mejorando la comprensión contextual en tareas como traducción y clasificación.",
        11: "Temperature, top-k, top-p, longitud máxima y penalización de repetición. Estos parámetros controlan creatividad, coherencia, precisión y diversidad de las salidas.",
        12: "Se puede adaptar mediante prompt engineering, RAG para ampliar conocimiento y fine-tuning para especialización profunda. Bedrock ofrece herramientas para estos métodos.",
        13: "SageMaker cubre preparación de datos, entrenamiento, tuning, deployment y monitoreo. Incluye Studio, pipelines, autoML y herramientas de MLOps para producción escalable.",
        14: "Se deben considerar el tipo de tarea, rendimiento, costo, latencia, idioma, seguridad, capacidad de instrucciones, alineación y evaluaciones del proveedor. AWS recomienda usar Model Evaluation Tool.",
        15: "Porque permite entender cómo funciona el modelo, sus datos y limitaciones. Facilita auditorías, cumplimiento y confianza, y ayuda a mitigar riesgos y sesgos."    
        }
...
Compara la respuesta esperada (texto) con la respuesta del estudiante y evalúa la calidad.

INSTRUCCIONES:
- Devuelve SOLO un JSON válido con EXACTAMENTE estos campos: {{
    "question": {k},
    "score": X,          # número decimal entre 0 y {per_q} con 2 decimales
    "feedback": "Texto corto de retroalimentación (1-2 frases)."
  }}
...
with tabs[2]:
    ...
    st.markdown(
    """
    📥 [Abrir un taller de deployment (Examen_ID_NRC.ipynb)](https://github.com/adiacla/piano)
...
with tabs[3]:
    ...
    preguntas_abiertas = [
        "¿Para qué sirve el parámetro return_state en una LSTM en Keras?",
        "¿Qué función tiene una capa Embedding?",
        "¿Por qué no se debe usar one-hot encoding para secuencias largas?",
        "¿Qué es un many-to-one RNN?",
        "¿Qué es un many-to-many RNN?",
        "¿Qué significa que LSTM tenga memoria a largo plazo?",
        "¿Por qué las RNN no se paralelizan bien?",
        "¿Qué es un Autoencoder?",
        "¿Qué son las capas Encoder y Decoder en un autoencoder?",
        "Explica cómo el mecanismo de self-attention dentro de los Transformers beneficia las tareas de NLP.",
        "¿Qué parámetros de configuración de un LLM influyen en la creatividad o precisión de sus respuestas?",
        "¿Cómo se puede adaptar un LLM para tareas específicas dentro de un proyecto de NLP?",
        "Explica cómo Amazon SageMaker soporta el ciclo de vida completo de Machine Learning.",
        "¿Qué factores se deben considerar al seleccionar un Foundation Model en Amazon Bedrock?",
        "Explica por qué la transparencia es un pilar clave en iniciativas de AI Responsible."
    ]
